...
//for the piezosensor
const int threshold = 1022; //needs to change based on testing
...
//    int NUMLIGHTED = int((double((now.hour()%12)/12))*60);
//    int NUMLIGHTED = 60;
...
//    Serial.println(now.minute());
    ...
    if(now.hour() == 20 && now.minute() == 21 && now.second() == 0) {
      bool x = true;
      int min = now.minute();
      ...
      while (x == true) {
        ...
        if (reading >= threshold) {
          ...
          x = false;
          buzzer = 9;
          NUMLIGHTED = NUMPIXELS/(round(double((720.0/((now.hour()%12)*60.0 + double(now.minute()))))));
          for (int i = 0; i < NUMLIGHTED; i ++) {
            pixels.setPixelColor(i, pixels.Color(137, 207, 240));
            pixels.show();
          }
